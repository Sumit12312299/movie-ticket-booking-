    ...
    >
      ...
      <div className="p-2.5 flex-1 flex flex-col justify-between space-y-2">
        <div>
          <Link
            to={`/movie/${movie.id}`}
            className="text-xs sm:text-sm font-black text-slate-900 dark:text-white hover:text-rose-600 dark:hover:text-rose-450 transition-colors line-clamp-1 block tracking-tight"
          >
            {movie.title}
          </Link>

          <div className="flex items-center gap-1.5 text-[10px] sm:text-[11px] text-slate-500 dark:text-slate-405 mt-1 font-bold">
          ...
          <div className="flex flex-wrap gap-1 mt-1.5">
            {(Array.isArray(movie.genre) ? movie.genre : []).slice(0, 2).map((g, idx) => (
              <span
                key={idx}
                className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-[9px] sm:text-[10px] font-bold text-slate-650 dark:text-slate-350 border border-slate-200/60 dark:border-slate-700/50"
              >
                {g}
              </span>
            ))}
        ...
        <div className="pt-2 border-t border-slate-100 dark:border-slate-800/60 flex items-center gap-1.5">
          ...
          <Link
            ...
            className="flex-1 py-2 rounded-xl bg-gradient-to-r from-red-600 to-rose-500 hover:from-red-500 hover:to-rose-400 text-white font-extrabold text-[10px] sm:text-xs text-center flex items-center justify-center gap-1 shadow-md shadow-rose-600/30 transition-all hover:scale-[1.02] active:scale-[0.98]"
          >

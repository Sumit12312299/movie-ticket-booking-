...
import { DollarSign, Ticket, Film, Users, Plus, Trash2, Edit3, ShieldAlert, BarChart3, TrendingUp, Calendar, MapPin } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  return (
    <div className="space-y-8 pb-16">
      ...
      <div className="glass-panel p-6 rounded-3xl border border-amber-500/30 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Real-time revenue metrics, catalog CRUD, & system operations</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-2xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs flex items-center gap-2 shadow-lg transition-all"
          >
      ...
      {stats && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="glass-card p-6 rounded-3xl border border-slate-800 space-y-2">
            ...
            <p className="text-3xl font-black text-white font-mono">${stats.total_revenue.toFixed(2)}</p>
            <span className="text-[10px] text-emerald-400 font-semibold">+18.5% this month</span>
          </div>
          ...
          <div className="glass-card p-6 rounded-3xl border border-slate-800 space-y-2">
            ...
            <p className="text-3xl font-black text-white font-mono">{stats.total_tickets_sold}</p>
            <span className="text-[10px] text-slate-400 font-semibold">Across all screens</span>
          </div>

          <div className="glass-card p-6 rounded-3xl border border-slate-800 space-y-2">
            <div className="flex items-center justify-between text-amber-400">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Active Movies</span>
              <Film className="w-5 h-5" />
            </div>
            <p className="text-3xl font-black text-white font-mono">{stats.total_movies}</p>
            <span className="text-[10px] text-amber-300 font-semibold">Now Showing & Upcoming</span>
          </div>
          ...
          <div className="glass-card p-6 rounded-3xl border border-slate-800 space-y-2">
            ...
            <p className="text-3xl font-black text-white font-mono">{stats.total_users}</p>
            <span className="text-[10px] text-slate-400 font-semibold">Customer Accounts</span>
          </div>
        </div>
      )}
                ...
                return (
                  <div key={genre} className="space-y-1">
                    <div className="flex justify-between text-xs font-semibold">
                      ...
                      <span className="text-amber-400 font-mono">${amount.toFixed(2)} ({pct.toFixed(0)}%)</span>
                    </div>
          ...
          <div className="glass-card rounded-3xl p-6 border border-slate-800 space-y-4">
            ...
            <div className="space-y-3 max-h-72 overflow-y-auto">
              {stats.recent_bookings?.map((b) => (
                <div key={b.id} className="p-3 rounded-xl bg-slate-900/60 border border-slate-800 flex items-center justify-between text-xs">
                  <div>
                    <span className="font-bold text-white block">{b.user_name}</span>
                    <span className="text-slate-400 text-[10px]">{b.movie_title} • {b.seats.join(', ')}</span>
                  </div>
                  <span className="font-mono font-bold text-emerald-400">${b.total_amount.toFixed(2)}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {activeTab === 'movies' && (
        <div className="glass-card rounded-3xl p-6 border border-slate-800 space-y-4">
          <h3 className="text-sm font-bold text-white uppercase tracking-wider">Movie Inventory</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-300">
              <thead className="bg-slate-900 uppercase text-[10px] font-bold text-slate-400">
                <tr>
              ...
              <tbody className="divide-y divide-slate-800/80">
                {movies.map((m) => (
                  <tr key={m.id} className="hover:bg-slate-900/40">
                    ...
                    <td className="p-3 font-mono font-bold text-amber-400">★ {m.rating.toFixed(1)}</td>
                    <td className="p-3">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-xl animate-slide-up">
          <div className="relative max-w-xl w-full glass-card p-6 rounded-3xl border border-rose-500/40 max-h-[90vh] overflow-y-auto space-y-4">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <h3 className="text-lg font-bold text-white">Create New Movie Record</h3>
              <button onClick={() => setShowAddMovie(false)} className="text-slate-400 hover:text-white">✕</button>
            </div>

            <form onSubmit={handleCreateMovie} className="space-y-3 text-xs">
              <div>
                <label className="font-semibold text-slate-400 block mb-1">Movie Title</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="font-semibold text-slate-400 block mb-1">Genre (Comma separated)</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="font-semibold text-slate-400 block mb-1">Duration (Mins)</label>
                  <input
                    type="number"
              ...
              <div>
                <label className="font-semibold text-slate-400 block mb-1">Poster URL</label>
                <input
                  type="text"
      ...
      {showAddShowtime && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-xl animate-slide-up">
          <div className="relative max-w-md w-full glass-card p-6 rounded-3xl border border-amber-500/40 space-y-4">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <h3 className="text-lg font-bold text-white">Schedule Showtime Slot</h3>
              <button onClick={() => setShowAddShowtime(false)} className="text-slate-400 hover:text-white">✕</button>
            </div>
            ...
            <form onSubmit={handleCreateShowtime} className="space-y-3 text-xs">
              ...
              <div>
                <label className="font-semibold text-slate-400 block mb-1">Theater Venue</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="font-semibold text-slate-400 block mb-1">Time Slot</label>
                  <input
                    type="text"
